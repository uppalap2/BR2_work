function [params_opti] = getParams_g_r(exp_data,~,WpL,F_e,n,Kmax,Tmin,EImin,GJmin)
...
fun = @(x)(10*((x(2)/Kmax)-1)^2 + ((x(4)/Tmin)-1)^2 + ((x(1)/EImin-1)^2+(x(2)/GJmin-1)^2));
% fun = @(x)1;
...
% % % for fitting
lb = [EImin;GJmin;-Kmax;-1];
ub = [10*EImin;10*GJmin;-Kmax;Tmin+1];
...
c1 = sqrt(dist(end,3))-5e-3;% 1 cm % for 24 psi
c2 = sqrt(dist(1:end-1,3))-10e-3;% 20e-3  for P2 and P3
