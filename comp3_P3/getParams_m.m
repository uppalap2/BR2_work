function [params_opti] = getParams_m(exp_data,~,WpL,F_e,n,Kmax,Tmin,EImin,GJmin)

% uses multistart to solve the problem
...
% % % for fitting
lb = [EImin/10;GJmin/10;-Kmax;-2];
ub = [100*EImin;100*GJmin;0;10];
...
problem = createOptimProblem('fmincon','x0',x0,'objective',fun,'lb',lb,'ub',ub,'nonlcon',@(x)nonlincon(x,WpL,...
                        ...
                        n_t_1,...
                        n_f,...
                        0,n,F_e,init_g_value,option,rexp,sexp),'options',options1);

% params_opti = [params_opti(1) params_opti(2) params_opti(3) params_opti(4)];
params_opti = run(ms,problem,50);
...
c1 = sqrt(dist(end-5:end,3))-10e-3;% 1 cm % for 24 psi
