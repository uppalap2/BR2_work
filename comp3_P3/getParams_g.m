...
% % % for fitting
% lb = [.8*EImin;.8*GJmin;-Kmax-1;-Tmin-3];
lb = [EImin;GJmin;-Kmax-1;-Tmin-3];
ub = [100*EImin;100*GJmin;0;4];
...
[shape_final] = cosserat_full_mod(params_opti,WpL,F_e,L,gravity_on,n_t);
...
[shape]  = cosserat_full_mod(x,WpL,F_e,L,gravity_on,n_t,[zeros(1,3) 1.0000 zeros(1,3) 1 zeros(1,3) 1],init_shape);
...
c1 = sqrt(dist(end,3))-15e-3;% 1 cm % for 24 psi
c2 = sqrt(dist(1:end-1,3))-25e-3;
% c2 = sqrt(sum((TanEnd_anal-tan_exp').^2))-1e-3;
% c = [c1 ; c2*0];
c = [1*c1;c2];
